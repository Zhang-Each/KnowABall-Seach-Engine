...
def player_news_titles_sql(id, name, english_name, news_titles, news_urls, news_img_urls):
    news_titles = news_titles.replace('"', "\'")
    sql = "INSERT INTO PLAYERNEWS VALUES(NULL, %s, \"%s\", \"%s\", \"%s\", \"%s\", \"%s\")" \
...
def team_player_news_list_sql(league, team_id):
    ...
    sql_list = []
    filename = '2person_news_list_' + str(league) + "_" + str(team_id) + '.csv'
    first = True
    with open(filename, encoding='utf-8')as f:
        ...
        for row in f_csv:
            ...
            news_urls = row[4]
            news_img_urls = row[5]
            sql = player_news_titles_sql(id, name, english_name, news_titles, news_urls, news_img_urls)
            sql_list.append(sql)
    return sql_list
